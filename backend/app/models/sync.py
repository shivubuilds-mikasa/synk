...
class ClipboardPayload(BaseModel):
    ...
    text: Annotated[
        ...
        Field(
            min_length=1,
            max_length=100000,  # 100KB max for MVP
...
class HistorySyncRequestMessage(SyncMessageBase):
    ...
    type: Literal[SyncMessageType.HISTORY_SYNC_REQUEST] = SyncMessageType.HISTORY_SYNC_REQUEST
    payload: Annotated[
        dict,
        Field(
...
ClientMessage = Union[ClipboardUpdateMessage, PingMessage, DeliveryReceiptMessage, HistorySyncRequestMessage]


class ClipboardUpdateReceivedMessage(SyncMessageBase):
    """Server -> Client: Acknowledgment that clipboard update was received."""

    type: Literal[SyncMessageType.CLIPBOARD_UPDATE_RECEIVED] = SyncMessageType.CLIPBOARD_UPDATE_RECEIVED
...
class ClipboardUpdateRelayMessage(SyncMessageBase):
    ...
    type: Literal[SyncMessageType.CLIPBOARD_UPDATE_RELAY] = SyncMessageType.CLIPBOARD_UPDATE_RELAY
    payload: ClipboardPayload
...
class HistorySyncResponseMessage(SyncMessageBase):
    ...
    type: Literal[SyncMessageType.HISTORY_SYNC_RESPONSE] = SyncMessageType.HISTORY_SYNC_RESPONSE
    payload: Annotated[
        dict,
        Field(
            description="History sync response data",
        ),
    ]
...
class DeviceOnlineMessage(SyncMessageBase):
    ...
    payload: Annotated[
        dict,
        Field(
            description="Online device info",
        ),
    ]
...
class DeviceOfflineMessage(SyncMessageBase):
    ...
    payload: Annotated[
        dict,
        Field(
            description="Offline device info",
        ),
    ]
...
class MessageValidator:
    ...
    MAX_MESSAGE_SIZE = 200000  # 200KB max message size
    MAX_TEXT_SIZE = 100000  # 100KB max text content
    ...
    def validate_raw_message(cls, raw: str) -> dict:
        ...
        Raises:
            ValueError: If message is too large or invalid JSON.
        """
        if len(raw) > cls.MAX_MESSAGE_SIZE:
            raise ValueError(f"Message too large: {len(raw)} bytes (max {cls.MAX_MESSAGE_SIZE})")
