...
async def clear_registries():
    """Clear both device and pairing registries before each test."""
    await device_registry.clear()
    await pairing_registry.clear()
    yield
    await device_registry.clear()
    await pairing_registry.clear()
...
class TestPairingCreate:
    ...
    @pytest.mark.asyncio
    async def test_create_pairing_code_success(self):
    ...
    @pytest.mark.asyncio
    async def test_create_pairing_code_unknown_device(self):
    ...
    @pytest.mark.asyncio
    async def test_create_pairing_code_missing_device_id(self):
    ...
    @pytest.mark.asyncio
    async def test_create_pairing_code_empty_device_id(self):
...
class TestPairingConfirm:
    ...
    @pytest.mark.asyncio
    async def test_confirm_pairing_unknown_confirming_device(self):
    ...
    @pytest.mark.asyncio
    async def test_confirm_pairing_expired_code(self):
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            ...
            # Access the pending pairing and set its expiration to past
    ...
    @pytest.mark.asyncio
    async def test_confirm_pairing_reused_code(self):
    ...
    @pytest.mark.asyncio
    async def test_confirm_pairing_self_pairing(self):
    ...
    @pytest.mark.asyncio
    async def test_confirm_pairing_duplicate_pairing(self):
    ...
    @pytest.mark.asyncio
    async def test_confirm_pairing_invalid_code_format(self):
    ...
    @pytest.mark.asyncio
    async def test_confirm_pairing_code_with_letters(self):
...
class TestGetPairedDevices:
    ...
    @pytest.mark.asyncio
    async def test_get_paired_devices_empty(self):
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            ...
            response = await client.get(f"/api/v1/pairing/{device_a_id}")
    ...
    @pytest.mark.asyncio
    async def test_get_paired_devices_after_pairing(self):
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            ...
            response_a = await client.get(f"/api/v1/pairing/{device_a_id}")
            # Get paired devices for Device B
            response_b = await client.get(f"/api/v1/pairing/{device_b_id}")
    ...
    @pytest.mark.asyncio
    async def test_get_paired_devices_unknown_device(self):
        """Test retrieving paired devices for unknown device returns 404."""
        import uuid
        fake_device_id = str(uuid.uuid4())

        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            response = await client.get(f"/api/v1/pairing/{fake_device_id}")
...
class TestMultipleIndependentPairings:
    ...
    @pytest.mark.asyncio
    async def test_multiple_devices_can_pair_independently(self):
        """Test that multiple device pairs can exist independently."""
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            # Register 4 devices
            reg_a = await client.post("/api/v1/devices/register", json={"device_name": "A", "device_type": "mobile"})
            reg_b = await client.post("/api/v1/devices/register", json={"device_name": "B", "device_type": "desktop"})
            reg_c = await client.post("/api/v1/devices/register", json={"device_name": "C", "device_type": "mobile"})
            reg_d = await client.post("/api/v1/devices/register", json={"device_name": "D", "device_type": "desktop"})
            ...
            # Pair A with B
            create_ab = await client.post("/api/v1/pairing/create", json={"device_id": device_a_id})
            code_ab = create_ab.json()["code"]
            await client.post("/api/v1/pairing/confirm", json={"device_id": device_b_id, "code": code_ab})

            # Pair C with D
            create_cd = await client.post("/api/v1/pairing/create", json={"device_id": device_c_id})
            code_cd = create_cd.json()["code"]
            await client.post("/api/v1/pairing/confirm", json={"device_id": device_d_id, "code": code_cd})

            # Verify pairings
            paired_a = await client.get(f"/api/v1/pairing/{device_a_id}")
            paired_b = await client.get(f"/api/v1/pairing/{device_b_id}")
            paired_c = await client.get(f"/api/v1/pairing/{device_c_id}")
            paired_d = await client.get(f"/api/v1/pairing/{device_d_id}")
    ...
    @pytest.mark.asyncio
    async def test_one_device_can_pair_with_multiple(self):
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            ...
            create_ac = await client.post("/api/v1/pairing/create", json={"device_id": device_a_id})
            code_ac = create_ac.json()["code"]
            await client.post("/api/v1/pairing/confirm", json={"device_id": device_c_id, "code": code_ac})
...
class TestPairingCodeSecurity:
    ...
    @pytest.mark.asyncio
    async def test_pairing_code_is_6_digits(self):
        """Test that generated pairing codes are exactly 6 digits."""
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            reg = await client.post("/api/v1/devices/register", json={"device_name": "Test", "device_type": "mobile"})
    ...
    @pytest.mark.asyncio
    async def test_pairing_code_is_random(self):
    ...
    @pytest.mark.asyncio
    async def test_pairing_code_uses_secrets_module(self):
...
class TestPairingExpiration:
    ...
    @pytest.mark.asyncio
    async def test_pairing_code_expires_after_5_minutes(self):
        """Test that pairing codes expire after 5 minutes."""
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            reg_a = await client.post("/api/v1/devices/register", json={"device_name": "A", "device_type": "mobile"})
            device_a_id = reg_a.json()["device_id"]

            create = await client.post("/api/v1/pairing/create", json={"device_id": device_a_id})
            code = create.json()["code"]
    ...
    @pytest.mark.asyncio
    async def test_expired_code_cleanup(self):
...
class TestPairingEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_confirm_with_nonexistent_code(self):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_pairing_attempts(self):
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            ...
            confirm_b = client.post("/api/v1/pairing/confirm", json={"device_id": device_b_id, "code": code})
            confirm_c = client.post("/api/v1/pairing/confirm", json={"device_id": device_c_id, "code": code})
    ...
    @pytest.mark.asyncio
    async def test_pairing_persists_after_confirm(self):
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            ...
            await client.post("/api/v1/pairing/confirm", json={"device_id": device_b_id, "code": code})
