...
import 'package:synk_mobile/features/clipboard/clipboard_sync_screen.dart';
...
class _AuthGateState extends State<_AuthGate> {
  ...
  Future<void> _checkRegistrationStatus() async {
    ...
    setState(() {
      _isLoading = false;
      if (isRegistered) {
        _child = const _LoadingClipboardSync();
      } else {
...
/// Widget that loads and displays clipboard sync after registration.
class _LoadingClipboardSync extends StatefulWidget {
  const _LoadingClipboardSync();

  @override
  State<_LoadingClipboardSync> createState() => _LoadingClipboardSyncState();
}

class _LoadingClipboardSyncState extends State<_LoadingClipboardSync> {
  @override
  void initState() {
    super.initState();
    _loadAndNavigate();
  ...
  Future<void> _loadAndNavigate() async {
    ...
    try {
      await deviceService.getCurrentDevice();
      if (!mounted) return;

      Navigator.of(context).pushReplacement(
        MaterialPageRoute(
          builder: (_) => const ClipboardSyncScreen(),
        ),
      );
    } catch (e) {
      if (!mounted) return;
